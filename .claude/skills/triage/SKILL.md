...
**The `triage` label is the work queue** — not just what this run filed. Anyone can apply it
to any issue, and anything wearing it eventually gets picked up. Issues filed by this run
enter the same queue as everything else; they do not jump it.
...
Then drop anything already carrying `investigated`. Order what remains by:

1. Impacted users in the last 24h (from the issue's Impact table)
2. Whether it appears in the current release — `version.properties` is the source of truth
3. Age, oldest first, as the tiebreak so nothing starves

Run `/investigate_issue <n>` on the **top two**.

### Skip, don't investigate

- **Diagnostics-only items** — empty stack, no app-code frame. There is nothing to
  implement, and a speculative patch is worse than none.
- Anything already labelled `investigated`.
- Issues where a human has asked a question in the comments and is waiting on an answer.
  Leave those alone; a bot talking over a maintainer is worse than a slow queue.

### Keep the backlog visible

Cap at two per run so runtime stays bounded. **Always report how many remain queued** — a
backlog that grows silently is indistinguishable from one that is being worked.

To park an issue without it being picked up again, remove the `triage` label. That is the
only off switch, and it is deliberate: there is no second "ignore me" label to forget about.
