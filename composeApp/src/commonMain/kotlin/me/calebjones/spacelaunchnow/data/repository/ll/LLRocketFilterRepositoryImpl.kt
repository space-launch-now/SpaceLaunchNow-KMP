...
/**
 * Rocket filter-option repository, backed directly by the Launch Library 2 (LL) API - the
 ...
 * CURRENT [RocketFilterRepository] interface, unchanged by the Trantor migration.
 *
 * Mirrors the pre-migration structure exactly: two independent LL endpoint calls
 * (`ProgramsApi.getProgramList` and `LauncherConfigurationFamiliesApi.launcherConfigurationFamiliesList`)
 * with a simple in-memory cache per list, no persistent local cache involved (this repository
 * never had one, before or after the Trantor migration).
 */
class LLRocketFilterRepositoryImpl(
