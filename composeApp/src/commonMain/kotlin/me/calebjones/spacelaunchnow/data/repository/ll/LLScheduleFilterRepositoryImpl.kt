...
/**
 * Schedule filter-option repository, backed directly by the Launch Library 2 (LL) API - the
 ...
 * CURRENT [ScheduleFilterRepository] interface, unchanged by the Trantor migration.
 *
 * Trantor collapsed the statuses/orbits/mission-types picker calls into a single `GET /lookups`
 * aggregate; this restoration keeps the pre-migration shape of five independent LL endpoints
 * (agencies, programs, launcher configurations, locations, and `ConfigApi` for statuses/orbits/
 * mission types) plus the `LauncherConfigurationFamiliesApi` call, mirroring the structure and
 * stale-cache fallback behavior of the Trantor-backed
 * [me.calebjones.spacelaunchnow.data.repository.ScheduleFilterRepositoryImpl]. Every network
 * result is mapped LL -> [FilterOption] inline (no dedicated mapper existed for this pre-migration
 * shape either) before being cached through [FilterOptionsLocalDataSource].
 */
class LLScheduleFilterRepositoryImpl(
