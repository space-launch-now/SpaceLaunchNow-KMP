...
class FilterOptionsLocalDataSource(
    ...
    // Agency operations
    suspend fun cacheAgency(id: Int, name: String, abbreviation: String?, isFeatured: Boolean = true) {
    ...
    // Rocket operations
    suspend fun cacheRocket(id: Int, name: String) {
        val now = System.now().toEpochMilliseconds()
        val duration = getEffectiveCacheDuration()
        val expiresAt = now + duration.inWholeMilliseconds
    ...
    suspend fun cacheRockets(rockets: List<Pair<Int, String>>) {
        rockets.forEach { (id, name) -> cacheRocket(id, name) }
    }
    ...
    suspend fun cacheStatuses(statuses: List<Tuple4<Int, String, String?, String?>>) {
        statuses.forEach { (id, name, abbrev, description) -> cacheStatus(id, name, abbrev, description) }
    }
