...
/**
 * Main screen for browsing agencies.
 *
 * Features:
 * - Paginated list of agencies
 * - Infinite scroll loading
 * - Search by name
 * - Sort by name, launches, or founded year
...
fun AgencyListScreen(
    ...
    val viewModel = koinViewModel<AgencyListViewModel>()
    ...
    val activeFilterCount = remember(
        ...
        uiState.featuredFilter
    ) {
        var count = 0
        if (uiState.searchQuery.isNotBlank()) count++
        if (uiState.selectedSortOrder != AgencySortOrder.NAME_ASC) count++
        if (uiState.featuredFilter != true) count++
        count
    }
    ...
    Scaffold(
                    ...
                    IconButton(onClick = { showFilterSheet = true }) {
                        BadgedBox(
                            ...
                            Icon(
                                ...
                                contentDescription = "Filter agencies"
                            )
                        }
                    }
                },
        ...
        Box(
            ...
            when {
                // Error state
                uiState.error != null && uiState.agencies.isEmpty() -> {
                    ErrorContent(
                ...
                uiState.isLoading && uiState.agencies.isEmpty() -> {
                    LoadingContent()
                }

                // Empty state
                uiState.agencies.isEmpty() && !uiState.isLoading -> {
                    EmptyContent()
                }
                ...
                // Content loaded
                    ...
                    LazyColumn(
                        ...
                        items(
                            items = uiState.agencies,
                            key = { it.id }
                        ) { agency ->
                            AgencyListItem(
                                agency = agency,
                                onClick = { onNavigateToAgencyDetail(agency.id) }
                            )
                        }
    ...
    if (showFilterSheet) {
        ModalBottomSheet(
            ...
            AgencyFilterSheet(
                ...
                featuredFilter = uiState.featuredFilter,
                ...
                onFeaturedChange = { featured ->
                    viewModel.updateFeaturedFilter(featured)
                },
                onClearAll = {
