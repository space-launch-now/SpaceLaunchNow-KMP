    ...
    MessagingDelegate
{
    ...
    func application(
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handleNotificationPermissionGranted),
            name: NSNotification.Name("KotlinNotificationPermissionGranted"),
            object: nil
        )
    ...
    func application(
        ...
        print(
            "Background Refresh Status: \(backgroundRefreshStatusString(backgroundRefreshStatus))")
        ...
        let hasContentAvailable =
            (userInfo["aps"] as? [String: Any])?["content-available"] as? Int == 1
    ...
    private func parseNotificationData(from userInfo: [AnyHashable: Any]) -> NotificationData? {
        ...
        for (key, value) in userInfo {
            if let keyString = key as? String {
                ...
                if keyString == "aps" || keyString.hasPrefix("gcm.")
                    || keyString.hasPrefix("google.c.")
                {
                    continue
                }
    ...
    private func formatLaunchDate(_ launchNet: String) -> String {
        ...
        guard
            let date = inputFormatter.date(from: launchNet)
                ?? {
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                ...
                let attachment = try UNNotificationAttachment(
                    identifier: "image", url: tmpFile, options: nil)
    ...
    private func saveNotificationToHistory(
        ...
        if let jsonData = try? JSONSerialization.data(
            withJSONObject: userInfo, options: .prettyPrinted),
